...
const ActionList = (Props: props) => {
  ...
  // check if user participate to the event
  const userParticipate = event?.getEvent?.participants?.find(
    (participant) => participant.id === currentUser?.profile?.id
  );
  ...
  const handleCompleteActionClick = async (
    ActionPoints: undefined | Maybe<string>,
    ElementClicked: String | undefined
  ) => {
      ...
      <div className="d-flex flex-column gap-5 my-5">
        <div className="col-8 d-flex flex-column align-items-start gap-2">
          <h2 className="fw-bold text-body-emphasis">
            Pour chaque évènement, nous vous proposons une liste d'actions à
            réaliser
          </h2>
          <p className="text-body-secondary">
            Le but de chaque évènement est de vous sensibiliser à l'écologie sur
            des sujets auxquels vous n'auriez pas forcément pensé au quotidien.
            Pour vous aider le créateur de cet évènement vous propose une liste
